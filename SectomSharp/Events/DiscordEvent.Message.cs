...
#pragma warning disable CA1822
    public async Task HandleMessageDeletedAsync(
        Cacheable<IMessage, ulong> partialMessage,
        Cacheable<IMessageChannel, ulong> _
    )
#pragma warning restore CA1822
    {
        if (
            !partialMessage.HasValue
            || partialMessage.Value
                is not { Author.IsBot: false, Channel: IGuildChannel { Guild: { } guild } } message
        )
        {
            return;
        }
        ...
        [
            ...
            new(
                "Created At",
                TimestampTag.FormatFromDateTime(
        ...
        if (message.MentionedChannelIds.Count > 0)
        {
            entries.Add(
                new(
                    ...
                    String.Join(
                        ", ",
                        message.MentionedChannelIds.Select(MentionUtils.MentionChannel)
                    )
                )
            );
        }
        ...
        if (message.MentionedRoleIds.Count > 0)
        {
            entries.Add(
                new(
                    "Mentioned Roles",
                    String.Join(", ", message.MentionedRoleIds.Select(MentionUtils.MentionRole))
                )
            );
        }
        ...
        if (message.MentionedUserIds.Count > 0)
        {
            entries.Add(
                new(
                    "Mentioned Users",
                    String.Join(", ", message.MentionedUserIds.Select(MentionUtils.MentionUser))
                )
            );
        }
...
#pragma warning disable CA1822
    public async Task HandleMessageUpdatedAsync(
        Cacheable<IMessage, ulong> oldPartialMessage,
